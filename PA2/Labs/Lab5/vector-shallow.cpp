...
private:
    struct control_block {
        ...
        void add_ref() { ++m_ref; }
        void del_ref() { --m_ref; }
        size_t get_ref() const { return m_ref; }

        control_block * detach() {
            if (m_ref == 1)
                return this;

            control_block * new_block = new control_block;
            new_block->m_cap = m_cap;
            new_block->m_len = m_len;

            if (m_cap > 0) {
                new_block->m_data = new my_string[m_cap];
                std::copy(m_data, m_data + m_len, new_block->m_data);
            }
...
private:
    control_block * m_block = new control_block;
};
...
void my_vector::push_back(my_string const & item) {
    ...
    if (m_block->m_len >= m_block->m_cap) {
        reallocate(m_block->m_cap * 1.5 + 10);
    }
...
int main() {
    ...
    unsorted = tokens; // shallow copy: now both share same control block
    tokens = tokens;
    tokens.push_back({});
    std::sort(tokens.begin(), tokens.end(), my_string::less);

    print_tokens(tokens, "sorted/shared");
    print_tokens(unsorted, "unsorted/shared");
    return 0;
}
